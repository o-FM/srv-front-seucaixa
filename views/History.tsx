...
const History: React.FC<HistoryProps> = ({ sales }) => {
  ...
  if (selectedSale) {
    return (
      <div className="p-4 space-y-6 animate-in slide-in-from-right duration-300 pb-24 h-screen bg-black overflow-y-auto">
        ...
        <div className="rounded-[32px] overflow-hidden border border-zinc-800"><ThermalReceipt sale={selectedSale} /></div>
        <button onClick={() => window.print()} className="w-full py-5 bg-purple-600 rounded-2xl font-black text-white flex items-center justify-center gap-3 active:scale-95 shadow-lg shadow-purple-600/20"><Printer size={22} /> IMPRIMIR</button>
      </div>
    );
  }
